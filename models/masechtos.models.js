...
exports.sendMasechtosBySeder =(seder)=> 
        pool.query(`SELECT * FROM masechta_table WHERE seder_name = '${seder}';`)
        .then((res)=>{
            const {rows} = res;
...
exports.sendAllMasechtos =()=>
pool.query("SELECT * FROM masechta_table ORDER BY masechta_id;")
.then((res)=>{
    const {rows} = res;
 const newRows = changeMasechtaNames(rows,altMasechtaNames);   
      return newRows;
...
exports.sendSingleMasechtaDetails =(masechta)=>{
   ...
   return pool.query(`SELECT * FROM masechta_table WHERE masechta_name = '${masechtaName}'`)
          .then((res)=>{
    const {rows} = res;
    const newRows = changeMasechtaNames(rows,altMasechtaNames)
