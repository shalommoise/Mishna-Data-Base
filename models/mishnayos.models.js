...
const {altMasechtaNames, switchAltName, formatMishnayos} = require("../db/utils/utils");



exports.sendMultipleMishnayos = (masechta, perek, mishna)=> {
//     pool.connect()
// .then(()=>{
...
.then((res)=>{
const {rows} = res;
const newRows = formatMishnayos(rows,altMasechtaNames)
return newRows;
})

}
exports.sendMishnayosById = (start, end)=>{
    // pool.connect()
    //     .then(()=>{
            if(!end) end = start;
